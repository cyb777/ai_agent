...
from app.ai.schema.anlyze_response import AnlyzeResponse
...
class AnlyzeAgent:
    ...
    def answer(self, question: str, user_id: list):
        # 定义系统提示词
        prompt = """
        一:你是一个数据分析图表智能体,你有一个工具mysql_tool
        二:工作流程:请严格按照以下步骤执行
        1.查询数据，把数据以表格的形式存入表格数据
        2.根据问题做出详细的分析，按照以下格式分析，把分析结果存入分析结果
             详细分析:
                 1：xxxx:
                    xxxx
                    xxxx
             ...
             结论分析:
                 ...
                 xxxx
                 xxxx
        3.生成一个echarts图表，图表数据josn格式必须是以下要求
        返回的数据必须是一个可执行的json格式，其他文本信息不重要
        返回的图表需要具有保存功能
        三:重要规则
        1.sql查询规范
        ...
        agent = create_agent(
            ...
            response_format=AnlyzeResponse,
        )
        try:
            # 调用智能体回答问题
